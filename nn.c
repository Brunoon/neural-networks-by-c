...
matrix *feedforward(matrix *in) {
	...
	for (i = 0; i < num_layers-1; i++) {
		...
		deleteMatrix(a); //及时释放上一次的temp，以免占用内存过多；如果想要更快执行，可以注释该句
		a = temp;
	}
	return a;
}
...
int SGD(matrix *train_images[], matrix *train_labels[], int train_size, int epochs, int mini_batch_size, double eta,
		matrix *test_images[], matrix *test_labels[], int test_size) {
...
int backprop(matrix *x, matrix *y, matrix *nabla_w[], matrix *nabla_b[]) {
	...
	for (l = num_layers-2; l >= 1; l--) {
		...
		deleteMatrix(sp); //sp是一个临时矩阵，及时释放掉，以免内存泄露，更快执行可以注释该句,但不保证会耗尽进程的堆空间
		deleteMatrix(wt);
	}
...
int main() {
    ...
    char *filetrainimages = "/home/kinny/Study/demo/train_images.txt";
    char *filetrainlabels = "/home/kinny/Study/demo/train_labels.txt";
    char *filetestimages  = "/home/kinny/Study/demo/test_images.txt";
    char *filetestlabels  = "/home/kinny/Study/demo/test_labels.txt";;
    printf("reading data......\n");
	...
	*/
    ...
    SGD(train_images, train_labels, 60000, 30, 10, 0.5, test_images, test_labels, 10000);
